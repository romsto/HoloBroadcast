...
public class GreetingFlagHandler extends FlagValueChangeHandler<String> {
    ...
    public static void registerFlag() {
        ...
        try {
            StringFlag flag = new StringFlag("hb-enter-hud", "deny");
            registry.register(flag);
            hbflag = flag;
        } catch (FlagConflictException e) {
            Flag<?> existing = registry.get("hb-enter-hud");
    ...
    protected boolean onSetValue(LocalPlayer localPlayer, Location location, Location location1, ApplicableRegionSet applicableRegionSet, String currentValue, String lastValue, MoveType moveType) {
        if (currentValue != null && !currentValue.equals(lastValue) && Bukkit.getPlayer(localPlayer.getUniqueId()) != null && !currentValue.equals("deny")) {
            sendHUDtoPlayer(localPlayer, currentValue);
        }
        return true;
    }
